...
public class RuleEngineTest
{
  ...
  @AfterClass public static void measureResourcesAfterTests()
  {
    Map<Thread, StackTraceElement[]> currentThreads = Thread.getAllStackTraces();
  ...
  /**
   * Test switch event state modification on a DRL file (on --> off)
   *
  ...
  @Test public void testEventModRule() throws Exception
  {
    ...
    Assert.assertTrue(sw1.getValue().equals("off"));
    Assert.assertTrue(sw1.getState() == Switch.State.OFF);
  }
  ...
  @Test public void testMixedDeploymentWithErrors() throws Exception
  {
    ...
    Assert.assertTrue(evt.getSourceID() == 123);
    Assert.assertTrue(
        "Expected 321, got " + evt.getValue(),
        evt.getValue().equals(321)
    );

    Assert.assertTrue(evt.serialize().equals("321"));
    Assert.assertTrue(evt instanceof Range);
    Assert.assertTrue(!evt.equals(range));
    Assert.assertTrue(evt != range);
    ...
    Assert.assertTrue(evt.getValue().equals(55));
    Assert.assertTrue(evt.serialize().equals("55"));
    Assert.assertTrue(evt instanceof Range);
    Assert.assertTrue(!evt.equals(range));
    Assert.assertTrue(evt != range);
    ...
    Assert.assertTrue(
        "Expected 6666, got " + evt.getValue(),
        evt.getValue().equals(6666)
    );
    
    Assert.assertTrue(evt.serialize().equals("6666"));
    Assert.assertTrue(evt instanceof Range);
    Assert.assertTrue(!evt.equals(range));
    Assert.assertTrue(evt != range);
    ...
    Assert.assertTrue(
        "Expected -77, got " + evt.getValue(),
        evt.getValue().equals(-77)
    );
    
    Assert.assertTrue(evt.serialize().equals("-77"));
    Assert.assertTrue(evt instanceof Range);
    Assert.assertTrue(!evt.equals(range));
    Assert.assertTrue(evt != range);
   ...
   * @throws Exception  if test fails
   */
  @Test public void vacationTest() throws Exception
  {
    String newResourcePath = AllTests.getAbsoluteFixturePath()
   ...
   * @throws Exception  if test fails
   */
  @Test public void testSwitchModifications() throws Exception
  {
    String newResourcePath = AllTests.getAbsoluteFixturePath()
   ...
   * @throws Exception  if test fails
   */
  @Test public void testLevelCommandExecution() throws Exception
  {
   ...
   * @throws Exception  if test fails
   */
  @Test public void testScheduledCommandExecution() throws Exception
  {
    StatusCache cache = null;
  ...
  private static class CommandCounter implements CommandBuilder
  {
    ...
    @Override public org.openremote.controller.command.Command build(Element el)
    {
      count++;
  ...
  private static class TestCommand implements EventProducer
  {
